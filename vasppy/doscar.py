...
def pdos_column_names(lmax, ispin):
    if lmax == 2:
        ...
        names = ['s',
                 ...
                 'f_y(3x2-y2)', 'f_xyz', 'f_yz2', 'f_z3',
                 'f_xz2', 'f_z(x2-y2)', 'f_x(x2-3y2)']
    else:
...
class Doscar:
    ...
    def __init__(self, filename, ispin=2, lmax=2, lorbit=11,
                 spin_orbit_coupling=False, read_pdos=True, species=None):
        '''
    ...
    def read_atomic_dos_as_df(self, atom_number):
        assert atom_number > 0 & atom_number <= self.number_of_atoms
        start_to_read = Doscar.number_of_header_lines + \
            atom_number * (self.number_of_data_points + 1)
        df = pd.read_csv(self.filename,
                         skiprows=start_to_read,
                         nrows=self.number_of_data_points,
                         delim_whitespace=True,
    ...
    def pdos_select(self, atoms=None, spin=None, l=None, m=None):
        ...
        if not l:
            ...
            if not m:
                ...
                channel_idx = [i + 1 for i,
                               v in enumerate(valid_m_values['p']) if v in m]
        elif l == 'd':
            if not m:
                channel_idx = [4, 5, 6, 7, 8]
            else:  # TODO this looks like it should be i+4
                channel_idx = [i + 4 for i,
                               v in enumerate(valid_m_values['d']) if v in m]
        elif l == 'f':
            if not m:
                channel_idx = [9, 10, 11, 12, 13, 14, 15]
            else:  # TODO this looks like it should be i+9
                channel_idx = [i + 9 for i,
                               v in enumerate(valid_m_values['f']) if v in m]
        else:
            raise ValueError
    ...
    def plot_pdos(self, ax=None, to_plot=None, colors=None,
        ...
        for species in to_plot.keys():
            ...
            for state in to_plot[species]:
                ...
                up_dos = self.pdos_sum(
                    atoms=index, l=state, spin='up')[e_range]
                down_dos = self.pdos_sum(
                    atoms=index, l=state, spin='down')[e_range]
        ...
        if plot_total_dos:
            ...
            auto_ymax = max([auto_ymax, self.tdos.up.values[e_range].max(
        ...
        if title:
            ax.set_title(title, loc=title_loc, fontdict={
