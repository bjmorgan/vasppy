...
forces_re = re.compile(r"\s+[+-]?\d+\.\d+\s+[+-]?\d+\.\d+\s+[+-]?\d+\.\d+\s+([+-]?\d+\.\d+)\s+([+-]?\d+\.\d+)\s+([+-]?\d+\.\d+)")
...
class ForcesData():
    ...
    def __post_init__(self):
        if isinstance(self.forces, list): self.forces = np.array(self.forces)
        self.number_of_ions = self.forces.shape[0]
        force_magnitude = np.sqrt(np.sum(self.forces**2, axis=1))
        self.mean_excess_force = np.where(force_magnitude > self.convergence, 
                                          force_magnitude - self.convergence, 0).sum() / self.number_of_ions
...
def get_forces_data(outcar_filename="OUTCAR", convergence=None, warn=False):
    ...
    if not convergence:
        ...
        try:
            ...
            raise ValueError("Unable to read EDIFFG.")
        except:
            raise
    ...
    if not forces_block_is_well_formed(forces_lines):
        ...
        if warn:
            print("The last FORCES block is not well-formed. Trying to parse the preceeding block.") 
